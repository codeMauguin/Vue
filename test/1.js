 ...
 * @param {Array} source
 */
function updateArray(target, source, compare = (a, b) => a === b) {
  ...
  P: while (old_index < target.length && new_index < source.length) {
    ...
    for (let i = old_index; i < target.length; ++i) {
      if (compare(target[i], source[new_index])) {
        target[i] = null;
        old_index++;
        new_index++;
        continue P;
  ...
  target.filter((e) => e !== null).forEach((key) => del.push(key));
...
console.log(updateArray([1, 2, 3], [12, 12, 2, 4]));
