...
export default class ENode extends Node {
  ...
  render(context) {
    for (const [key, value] of Object.entries(this.$props.attributes)) {
      switch (key) {
        case "show": {
          const isShow = h(value, context["&data"]);
        ...
        case "key":
          break;
        default: {
          this.props.attributes[key] = h(value, context["&data"]);
        }
      }
    }
    for (const [key, value] of Object.entries(this.$props.method)) {
      ...
      const methodRegExp = /(?<name>\w+)?([(,])(?<arg>.*?)(?=[,)])/g;
      ...
      const fun = context.isVue
        ? h(name, context._methods)
        : h(name, context.global._methods);

      this.props.method[key] = () => {
        const _args = [];
        Array.from(args).forEach((arg) => {
          _args.push(
            context.isVue ? h(arg, context["&data"]) : h(arg, context),
          );
        });
  ...
  init() {
    ...
    for (const [key, value] of Object.entries(
      Object.assign(this.attributes, this.props.attributes),
    )) {
      setAttribute(dom, key, value);
    }
    for (const [/**@type{HTMLElementEventMap}*/ key, value] of Object.entries(
      this.props.method,
    )) {
  ...
  clone() {
    ...
    clone.children.forEach(
      (/** @type {{ parent: ENode; }} */ child) => (child.parent = clone),
    );
    return clone;
  }
}
