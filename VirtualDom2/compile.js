...
const forDynamicGeneration = ({
    ...
    // @ts-ignore
    const target = propsMustache(foredeck.groups["target"],
                                 view);
...
export function compile(node,
    ...
    if (node instanceof ENode) {
        node.static = !(
            Reflect.ownKeys(node.prop.attributes).length > 0 ||
            Reflect.ownKeys(node.prop.method).length > 0
        );
        //通过节点属性生成key +index
        ...
        node.key = node.key ?? calculateCode(node,
                                             index);
        //更新视图内容
        node.render(context);
...
function calculateCode(node,
    ...
    return (
        (index << 5) +
        ((Object.keys(node?.attributes ?? {}).length + 9) << 2) +
        ((Object.keys(node.prop.attributes).length + 8) << 3) +
        ((Object.keys(node.prop.method).length + 9) << 4)
    );
}
...
class Observer {
    ...
    $on(node,
        ...
        if (isNull(msg?.attr["if"])) {
            ...
            const res = propsMustache(msg?.attr["if"],
                                      context);
            if (res) {
                this.$destroy(node,
                              0);
                ...
                node.children.splice(msg?.index + index--,
                                     1);
            }
        }
        while (this.message.length > 0) {
            ...
            if (isNotNull(res?.attr["else-if"])) {
                if (propsMustache(res?.attr["else-if"],
                                  context)) {
                    break;
                } else {
                    node.children.splice(res.index + index--,
                                         1);
                }
            } else if (isNotNull(res?.attr["else"])) {
                if (index === 0) {
                    console.warn(
                        "Else tag should not be in the first place, if it is in the first place it will never show!",
                    );
                    node.children.splice(res.index + index--,
                                         1);
                    continue;
                ...
                console.warn(
                    `The label ${JSON.stringify(res?.attr)} should not appear here`,
                );
                // Wipe the user's ass
                node.children.splice(res.index + index--,
                                     1);
