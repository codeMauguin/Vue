...
class Processor {
  ...
  request(config) {
    ...
    let timer = new Promise((_resolve, reject) => {
      setTimeout(() => {
        reject({
          message: `The request has timed out ${timeout} ms time`,
          errorCode: 500,
        });
        about.abort();
      }, timeout);
    ...
    return Promise.race([
      timer,
      fetch(url, {
        body: filter["method"] === "GET" ? null : body,
        signal: about.signal,
        ...filter,
      }),
    ]).then(
      (suc) => {
        if (suc.status === 200)
          return this.responseTask.executor(suc, null, group);
        else {
      ...
      (reason) => {
        if (reason.errorCode === 500)
          return Promise.reject(this.requestTask.executor(null, reason, group));
