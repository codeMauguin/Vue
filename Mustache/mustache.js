...
export default function mustache(stencil, view) {
  return ProcessingText(stencil, view);
}
...
export function propsMustache(stencil, view) {
  try {
    return isMustache(stencil)
      ? mustache(stencil, view)
      : render(stencil, view);
  } catch (e) {
    return stencil;
  }
  ...
  if ((match = stencilRegexp.exec(text)) !== null) {
    do {
      ...
      if (match.index > index) {
        stencil.push([`${text.slice(index, match.index)}`, 0]);
      }
      stencil.push([match[1] ?? match[2], 1]);
      index = lastIndex;
    } while ((match = stencilRegexp.exec(text)) !== null);
    if (index < text.length) {
      stencil.push([`${text.slice(index, text.length)}`, 0]);
    }
    return stencil;
  } else return [[text, 0]];
}
...
export function ProcessingText(text, view) {
  ...
  if ((match = stencilRegexp.exec(text)) !== null) {
    do {
      ...
      stencil.push(render(match[1] ?? match[2], view));
      index = lastIndex;
    } while ((match = stencilRegexp.exec(text)) !== null);
    if (index < text.length) {
