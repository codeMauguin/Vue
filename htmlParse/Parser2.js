...
const startTagOpen = new RegExp(`(?<body>^<${noname})`);
...
function advanceBy(expect, tokenizer) {
  const adaptation = expect.map((i) => spec[i]);
  for (const [regExp, type] of adaptation) {
    let regExpExecArray = match(regExp, tokenizer);
...
class Parser {
  ...
  attributeLiteral() {
    ...
    P: while (!this.EOF()) {
      const { type, value: val } = advanceBy(
        [
      ...
      switch (type) {
        case tokenizer.attributes:
          ...
          const {
            groups: { key: dynamicKey = false },
          } = dynamicAttr.exec(key) ?? { groups: { key: false } };
          if (dynamicKey) {
            switch (dynamicKey) {
              case "class":
              case "style":
  ...
  ElementLiteral(tag) {
    if (!tag) {
      const {
        type,
        value: { groups: body },
      } = advanceBy([tokenizer.startTagOpen], this.value);
    ...
    } = advanceBy([tokenizer.EOF], this.value);
    if (type !== tokenizer.EOF) {
      warn(`Template compilation error: Tags with side effect`);
      throw new DOMException(
        `Template compilation error: Tags with side effect`,
      );
    } else {
      this.eat(body.length);
    }
  ...
  eat(len) {
    if (len === 0) {
      ...
      throw new DOMException(
        `no characters matched:${this.template.slice(this.cursor)}`,
      );
    }
    this.cursor += len;
  }
  ...
  childLiteral() {
    ...
    P: while (!this.EOF()) {
      const {
        ...
        [tokenizer.startTagOpen, tokenizer.EOF, tokenizer.textNode],
        this.value,
      );
      const { body } = groups;
      switch (type) {
